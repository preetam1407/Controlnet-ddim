...
def zero_convolution(
    ...
    # Created a 1×1 conv that initially does nothing (all weights/bias = 0).
    module = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding,  bias=True)
    # Zero out the kernel so the mapping is initially all zeros.
    nn.init.zeros_(module.weight)
    # Zero out the bias as mentioned
...
class ControlNetModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        control_down_block_res_samples = []
        # block_idx = 0
        for idx, res in enumerate(down_block_res_samples):
            # Applying the zero-init conv to each residual feature map
            zeroed = self.controlnet_down_blocks[idx](res)
            control_down_block_res_samples.append(zeroed)
        # At the bottleneck, do the same zero-conv to the mid-block output.
        mid_block_res_sample = self.controlnet_mid_block(sample)   

        # now replacing the original UNet residuals with our control signals. 
        down_block_res_samples = control_down_block_res_samples
